...
const navigation = [
  ...
  },
  {
    ...
    href: "/resource-hub",
    items: [
      { name: "Articles & News", href: "/pakistan/blog" },
...
const topNavLinks = [
  ...
  { name: "Articles", href: "/pakistan/blog" },
  { name: "Institution", href: "/institution" },
  { name: "Contact Us", href: "/pakistan/contact-us" },
  { name: "Login", href: "/login" },
];

export default function PakHeader() {
  ...
  return (
    <header className="w-full bg-white border-b border-gray-200 sticky top-0 z-50 shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        {/* Logo */}
        <Link href="/pakistan" className="flex items-center gap-2 group">
                ...
                <div className="bg-white rounded-xl shadow-xl border border-gray-100 p-2 space-y-1">
                  {nav.items.map((item) => (
                    <Link
                      key={item.name}
                      href={item.href}
                      className="block px-3 py-2 text-sm text-gray-600 rounded-lg hover:bg-primary-soft hover:text-primary transition-colors"
                    >
                      {item.name}
                    </Link>
                  ))}
