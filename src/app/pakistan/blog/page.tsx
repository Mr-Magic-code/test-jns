...
  title: "Blogs | JnS Education Pakistan",
...
async function getPakistanBlogs() {
  try {
    const [rows] = await pool.query(
      `SELECT blogs.*, blog_categories.name AS category_name 
       FROM blogs 
       LEFT JOIN blog_categories ON blogs.category_id = blog_categories.id 
       WHERE blogs.region = 'pakistan' AND blogs.status = 'published' 
       ORDER BY blogs.created_at DESC`
    );
    return rows as any[];
  } catch (error) {
    console.error('Error fetching pakistan blogs:', error);
    return [];
  }
}

export default async function PakistanBlogPage() {
  const blogs = await getPakistanBlogs();
    ...
    <main className="bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="text-center space-y-3">
          <h2 className="best-heading font-extrabold text-gray-900 tracking-tight">Pakistan Blogs & Insights</h2>
          <p className="text-base text-gray-600 max-w-2xl mx-auto">Latest updates, visa guidelines, and expert study abroad advice for students in Pakistan.</p>
        </div>

        {blogs.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center border border-gray-200 shadow-xs max-w-xl mx-auto">
            <p className="text-gray-500 text-sm font-medium">No Pakistan blogs found.</p>
          </div>
        ) : (
            ...
            {blogs.map((blog) => (
              <article key={blog.id} className="bg-white rounded-3xl overflow-hidden border border-gray-200/85 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
                ...
                <div className="p-6 pt-0">
                  <Link href={`/pakistan/blog/${blog.slug}`} className="inline-flex items-center gap-2 text-sm font-bold text-primary hover:text-primary transition-colors">
