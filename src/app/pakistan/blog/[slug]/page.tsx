...
// 🚀 Performance Optimization: React cache for Pakistan region
const getBlog = reactCache(async (slug: string) => {
  const [rows]: any = await pool.query('SELECT * FROM blogs WHERE slug = ? AND region = ?', [slug, 'pakistan']);
  return rows[0] || null;
});
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: blog.title,
      description: cleanDescription,
      url: `https://yourdomain.com/pakistan/blog/${blog.slug}`,
      images: [
        {
